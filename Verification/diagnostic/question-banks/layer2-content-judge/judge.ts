/**
 * The judging pipeline for a diagnostic staging batch.
 *
 * Two dispatch paths per row, decided by question_type:
 *  - MCQ/TFNG: blind-solve -> compare -> adjudicate only on disagreement,
 *    same shape as drills' judge.ts.
 *  - WRITING_PROMPT/SPEAKING_PROMPT: direct quality judgement, since there's
 *    no stored answer to blind-solve against.
 *
 * Plus one set-level check specific to LISTENING: the real audio file is
 * transcribed via the existing Gemini transcription helper and diffed
 * against the author's submitted transcript, since neither text-only checking
 * path can catch a transcript that doesn't actually match the recording.
 */
...
import { OPTION_KEYS, TFNG_ANSWERS, type DiagnosticCsvRow, type OptionKey } from '../shared/types';
import { groupBySetId, normalizeEnumCell, type SetGroup } from '../layer1-verifier/checks';
...
export interface JudgeDeps {
  ...
  useCache: boolean;
  stats: JudgeStats;
  /** Directory audio_file names are resolved against, for the Listening cross-check. */
  audioDir?: string;
  /** Optional: transcribe an audio file. Injectable so tests can stub it out. */
...
export function readQuestionView(row: DiagnosticCsvRow): QuestionView | null {
  if (isBlank(row.prompt_text)) return null;
  const type = normalizeEnumCell(row.question_type);

  const grounding = !isBlank(row.passage_text) ? row.passage_text : !isBlank(row.transcript) ? row.transcript : null;
  ...
  if (type === 'MCQ') {
    ...
    return { promptText: row.prompt_text, options, passageOrTranscript: grounding };
  }

  if (type === 'TFNG') {
    return { promptText: row.prompt_text, options: null, passageOrTranscript: grounding };
  }

  return null; // WRITING_PROMPT / SPEAKING_PROMPT go through the other path.
}

function storedAnswerOf(row: DiagnosticCsvRow): string | null {
  const raw = row.correct_answer.trim().toUpperCase();
  const type = normalizeEnumCell(row.question_type);
  if (type === 'MCQ') return (OPTION_KEYS as readonly string[]).includes(raw) ? raw : null;
  if (type === 'TFNG') return (TFNG_ANSWERS as readonly string[]).includes(raw) ? raw : null;
  return null;
}
...
export async function judgeAnswerRow(row: DiagnosticCsvRow, deps: JudgeDeps): Promise<AnswerRowJudgement> {
  const type = normalizeEnumCell(row.question_type);
  ...
  try {
    ...
    const adjudication = await ask(deps, `${label} adjudicate`, adjudicatePrompt(view, skillLabel, isTfng, stored, blind), raw =>
      parseAdjudication(raw, isTfng),
    );
...
export async function judgePromptRow(row: DiagnosticCsvRow, deps: JudgeDeps): Promise<PromptRowJudgement> {
  const type = normalizeEnumCell(row.question_type);
  ...
  const minWords = type === 'WRITING_PROMPT' ? Number(row.min_words) || null : null;

  try {
    const label = `line ${row.line}`;
    const { outcome, reasoning } = await ask(deps, label, promptQualityPrompt(row.prompt_text, skillLabel, minWords), parsePromptQuality);
...
// ---------------------------------------------------------------------------
// Listening: audio-vs-transcript cross-check (per set, not per row)
// ---------------------------------------------------------------------------

/**
 * Confirms the author's submitted transcript actually matches what's in the
 * audio file, using the existing Gemini transcription helper as an
 * independent cross-check — not as ground truth (see prompts.ts's header
 * comment on why ASR is treated as a signal, not a source of truth here).
 */
export async function crossCheckListeningAudio(group: SetGroup, deps: JudgeDeps): Promise<AudioCrossCheck | null> {
  const skill = normalizeEnumCell(group.rows[0]?.skill ?? '');
  if (skill !== 'LISTENING' || group.rows.length === 0) return null;

  const audioFile = group.rows[0].audio_file?.trim();
  const transcript = group.rows[0].transcript?.trim();
  if (!audioFile || !transcript) {
    return { setId: group.setId, autoTranscript: null, matchesSubmittedTranscript: null, detail: 'No audio_file/transcript to cross-check.' };
  }

  const transcribe = deps.transcribeAudio;
  if (!transcribe) {
    return { setId: group.setId, autoTranscript: null, matchesSubmittedTranscript: null, detail: 'Audio cross-check skipped (no transcription configured).' };
  }

  const audioPath = deps.audioDir ? path.join(deps.audioDir, audioFile) : audioFile;

  try {
    deps.stats.apiCalls += 1;
    const autoTranscript = await transcribe(audioPath);

    const compared = `Compare these two transcripts of the same audio clip. Ignore trivial differences ` +
      `in punctuation/capitalization/filler words. Do they agree on the SUBSTANTIVE content — the ` +
      `facts, numbers, and names a listener would need to answer comprehension questions?\n\n` +
      `TRANSCRIPT A (author-submitted):\n${transcript}\n\nTRANSCRIPT B (auto-transcribed from the audio):\n${autoTranscript}\n\n` +
      `Respond with JSON only: {"matches":true|false,"reasoning":"one or two sentences, naming any specific discrepancy"}`;

    const raw = await deps.limit(() => deps.client.complete({ label: `set ${group.setId} audio cross-check`, prompt: compared }));
    ...
    return {
      setId: group.setId,
      autoTranscript,
      matchesSubmittedTranscript: matches,
      detail: matches ? `Audio matches submitted transcript. ${reasoning}`.trim() : `MISMATCH — needs human audio review. ${reasoning}`.trim(),
    };
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    return {
      ...
      detail: `Could not cross-check: ${message.slice(0, 160)}. NOT verified — treat as unconfirmed, not as matching.`,
    };
  }
}
...
export async function judgeRowCached(
  row: DiagnosticCsvRow,
  cache: JudgementCache,
  deps: JudgeDeps,
): Promise<AnswerRowJudgement | PromptRowJudgement> {
  const type = normalizeEnumCell(row.question_type);
...
export async function judgeFile(filePath: string, deps: JudgeDeps): Promise<JudgedFile> {
  ...
  const answerRows = judged.filter((j): j is { row: DiagnosticCsvRow; judgement: AnswerRowJudgement } => isAnswerJudgement(j.judgement));
  const promptRows = judged.filter((j): j is { row: DiagnosticCsvRow; judgement: PromptRowJudgement } => !isAnswerJudgement(j.judgement));

  const { groups } = groupBySetId(loaded.rows);
