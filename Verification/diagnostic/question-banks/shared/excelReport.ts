/**
 * Writes the colored .xlsx Layer 1 report. Same visual conventions as drills'
 * report (Excel's Good/Neutral/Bad palette, one Summary sheet + one sheet per
 * file) so reading a diagnostic report feels identical to reading a drills one.
 */

import ExcelJS from 'exceljs';
import type { FileResult, Finding, RowOutcome, RunResult } from './types';
...
function sheetNameFor(file: FileResult, index: number, taken: Set<string>): string {
...
function writeFileSheet(workbook: ExcelJS.Workbook, file: FileResult, sheetName: string): void {
  ...
  sheet.addRow([`Sets: ${file.setResults.length}   Rows: ${file.rowResults.length}   Expected: ${file.expectedRowCount}   Outcome: ${file.outcome.toUpperCase()}`]);
  sheet.addRow([]);

  writeFindingsBlock(sheet, 'File/set-level findings', file.fileFindings);
...
function writeSummarySheet(workbook: ExcelJS.Workbook, run: RunResult, sheetNames: string[]): void {
  ...
  const title = sheet.addRow(['Layer 1 — Diagnostic Question Staging CSV Verifier']);
  ...
  const head = sheet.addRow(['Sheet', 'File', 'Sets', 'Rows', 'Expected', 'Pass', 'Warn', 'Fail', 'File-level issues', 'Outcome']);
  styleTableHeader(head, 10);
  [22, 58, 7, 7, 9, 7, 7, 7, 17, 10].forEach((w, i) => {
  ...
  run.files.forEach((f, i) => {
    ...
    const row = sheet.addRow([sheetNames[i], f.fileName, f.setResults.length, f.rowResults.length, f.expectedRowCount, pass, warn, fail, f.fileFindings.length, f.outcome.toUpperCase()]);
...
export async function writeRunReport(run: RunResult, outPath: string): Promise<string> {
  ...
  workbook.creator = 'Layer 1 Diagnostic Question Verifier';
