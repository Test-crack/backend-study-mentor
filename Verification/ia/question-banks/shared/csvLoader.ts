/**
 * Reads an IA question CSV into positional rows.
 *
 * Structurally identical to drills' loader (RFC-4180 via `csv-parse`, header
 * read as data so an embedded duplicate header is caught, never throws) but
 * built around the 12-column IA row shape, with a 13th `source_key` column
 * that is optional exactly the way drills' is.
 */
...
export function loadIACsv(filePath: string): LoadedCsv {
  ...
  for (const rec of dataRecords) {
    ...
    rows.push({
      ...
      difficulty: cell(cells, indexByColumn[2]),
      question_type: cell(cells, indexByColumn[3]),
...
// the way it is).
export { toCsvText, writeDrillCsv as writeIACsv, findCsvFiles } from '../../../drills/question-banks/shared/csvLoader';
