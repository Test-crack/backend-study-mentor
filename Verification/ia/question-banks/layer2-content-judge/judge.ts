/**
 * The judging pipeline for an IA question CSV.
 *
 * Dispatch per row, decided by question_type — ported from diagnostic's
 * judge.ts:
 *  - MCQ/TFNG: blind-solve -> compare -> adjudicate only on disagreement.
 *  - WRITING_PROMPT/SPEAKING_PROMPT: direct quality judgement (no stored
 *    answer to blind-solve against). SPEAKING_PROMPT uses the identical
 *    prompt-quality check as WRITING_PROMPT — no transcript exists at
 *    authoring time.
 *
 * Plus one set-level check for LISTENING, grouped by `passage_id`: the audio
 * is transcribed for manual reference. Unlike diagnostic, IAQuestion carries
 * no author-submitted transcript column, so there is nothing to diff the
 * auto-transcript against — see AudioCrossCheck's header note in types.ts.
 */
...
import { OPTION_KEYS, TFNG_ANSWERS, type IACsvRow, type OptionKey } from '../shared/types';
import { groupByPassageId, normalizeEnumCell, type PassageGroup } from '../layer1-verifier/checks';
...
export function readQuestionView(row: IACsvRow): QuestionView | null {
...
function storedAnswerOf(row: IACsvRow): string | null {
  const type = normalizeEnumCell(row.question_type);
  if (type === 'MCQ') {
    try {
...
export async function judgeAnswerRow(row: IACsvRow, deps: JudgeDeps): Promise<AnswerRowJudgement> {
  const type = normalizeEnumCell(row.question_type);
...
export async function judgePromptRow(row: IACsvRow, deps: JudgeDeps): Promise<PromptRowJudgement> {
  const type = normalizeEnumCell(row.question_type);
  ...
  try {
    const label = `line ${row.line}`;
    const { outcome, reasoning } = await ask(deps, label, promptQualityPrompt(row.prompt_text, skillLabel), parsePromptQuality);
...
export async function crossCheckListeningAudio(group: PassageGroup, deps: JudgeDeps): Promise<AudioCrossCheck | null> {
  const skill = normalizeEnumCell(group.rows[0]?.skill ?? '');
  if (skill !== 'LISTENING' || group.rows.length === 0) return null;

  const audioUrl = group.rows[0].audio_url?.trim();
  if (!audioUrl) {
    return { passageId: group.passageId, autoTranscript: null, matchesSubmittedTranscript: null, detail: 'No audio_url to transcribe.' };
  }

  const transcribe = deps.transcribeAudio;
  if (!transcribe) {
    return { passageId: group.passageId, autoTranscript: null, matchesSubmittedTranscript: null, detail: 'Audio transcription skipped (not configured).' };
  }
  ...
  try {
    ...
    return {
      ...
      detail:
        'Auto-transcribed for manual reference. IAQuestion has no author-submitted transcript ' +
...
export async function judgeRowCached(row: IACsvRow, cache: JudgementCache, deps: JudgeDeps): Promise<AnswerRowJudgement | PromptRowJudgement> {
  const type = normalizeEnumCell(row.question_type);
...
export async function judgeFile(filePath: string, deps: JudgeDeps): Promise<JudgedFile> {
  ...
  const answerRows = judged.filter((j): j is { row: IACsvRow; judgement: AnswerRowJudgement } => isAnswerJudgement(j.judgement));
  const promptRows = judged.filter((j): j is { row: IACsvRow; judgement: PromptRowJudgement } => !isAnswerJudgement(j.judgement));

  const { groups } = groupByPassageId(loaded.rows);
