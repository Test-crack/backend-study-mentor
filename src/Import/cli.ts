/**
 * Importer — writes verified drill questions into `drill_questions`.
 *
 * The only thing in this pipeline that writes to the database. Upserts on `source_key`,
 * so running it twice cannot create duplicates.
 *
 *   npm run drills:import -- --target dev --level beginner            (dry run)
 *   npm run drills:import -- --target dev --level beginner --confirm  (writes)
 *
 * Safety model, all deliberate:
 *
 *  - **Dry run is the default.** Writing takes `--confirm`. Matches every other
 *    DB-touching script in this repo.
 *  - **`--target dev|prod` is required** and the resolved connection's database name is
 *    checked against it. Naming a target you are not connected to is refused.
 *  - **Layer 1 runs inline as a hard gate**, with `--require-source-key`. It is free and
 *    deterministic, so there is no reason to trust a stale report instead of just
 *    re-checking. A file with any failing finding is not imported.
 *  - **Layer 2 cannot be re-run cheaply** (it costs API calls), so a real write requires
 *    `--layer2-reviewed` — an explicit assertion by the operator, recorded in the output.
 *
 * Exit codes:
...
// Loads .env into process.env. Prisma reads .env on its own, but the target check below
// inspects process.env directly and would otherwise see nothing and refuse every run.
...
function resolveInputFiles(opts: CliOptions, positional: string[]): string[] {
  ...
  if (opts.level !== undefined) {
    ...
    if (level === null) {
      throw new UsageError(
        `--level must be beginner, intermediate or advanced — got "${opts.level}".`,
      );
    }
    ...
    if (!fs.existsSync(dir)) {
      throw new UsageError(
        `No tagged files for ${level.toLowerCase()} at:\n  ${dir}\n` +
          `  Run: npm run drills:assign-keys -- --level ${level.toLowerCase()}\n` +
...
async function main(): Promise<number> {
  ...
  program
    .name('drills:import')
    .description(
      'Imports verified, tagged drill questions into drill_questions. Upserts on ' +
        'source_key. Dry run unless --confirm is passed.',
    )
    ...
    .option('--level <name>', 'Import the tagged files for one level')
    ...
    .option(
      '--expected <n>',
      'Expected rows per file for the Layer 1 gate; a number or "advanced=50,else=200"',
  ...
  if (confirm && opts.layer2Reviewed !== true) {
    throw new UsageError(
      'Refusing to write without --layer2-reviewed.\n' +
        ...
        '    npm run drills:judge -- --level <level>',
    );
  }

  console.log('═══════════════════════════════════════════════════════════');
  console.log('  Importer — drill_questions');
