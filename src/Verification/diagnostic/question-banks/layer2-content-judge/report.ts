/**
 * Layer 2's colored Excel report. Same shape as Layer 1's, plus a dedicated
 * Listening audio cross-check block per set, since that's a finding that
...
function writeFileSheet(workbook: ExcelJS.Workbook, file: JudgedFile, sheetName: string): void {
  ...
  if (file.audioCrossChecks.length > 0) {
    const heading = sheet.addRow(['Listening audio cross-checks (per set)']);
    heading.font = { bold: true, size: 12 };
    const head = sheet.addRow(['Set', 'Matches transcript?', 'Detail']);
    styleTableHeader(head, 3);
    for (const check of file.audioCrossChecks) {
      const severity: JudgeSeverity = check.matchesSubmittedTranscript === null ? 'unknown' : check.matchesSubmittedTranscript ? 'ok' : 'defect';
      const row = sheet.addRow([check.setId, check.matchesSubmittedTranscript === null ? 'UNKNOWN' : check.matchesSubmittedTranscript ? 'YES' : 'NO — MISMATCH', check.detail]);
      paintRow(row, severity, 3);
    }
    sheet.addRow([]);
...
function writeSummarySheet(workbook: ExcelJS.Workbook, run: JudgeRunResult, sheetNames: string[]): void {
  ...
  const title = sheet.addRow(['Layer 2 — Diagnostic Question Content Judge']);
  ...
  const meanings: Array<[string, string]> = [
    ...
    ['TOO_EASY', "Doesn't discriminate students — the diagnostic needs harder content here."],
  ...
  for (const [outcome, meaning] of meanings) {
    sheet.addRow([outcome, meaning]);
  }
  sheet.addRow([]);

  const head = sheet.addRow(['Sheet', 'File', 'MCQ/TFNG rows', 'Prompt rows', 'Audio mismatches']);
  ...
  run.files.forEach((f, i) => {
    const mismatches = f.audioCrossChecks.filter(c => c.matchesSubmittedTranscript === false).length;
    ...
    const row = sheet.addRow([sheetNames[i], f.fileName, f.answerRows.length, f.promptRows.length, mismatches]);
...
export async function writeJudgeReport(run: JudgeRunResult, outPath: string): Promise<string> {
  ...
  workbook.creator = 'Layer 2 Diagnostic Question Content Judge';
