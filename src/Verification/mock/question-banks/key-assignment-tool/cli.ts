/**
 * Key-assignment tool — stamps a permanent `source_key` onto every Mock question.
 *
 * Ported from IA's cli.ts. Reads the CSVs an author produced, writes tagged
 * copies to `Verification/mock/results/key-assignment-tool/`. No difficulty
 * subfolder (Mock has none) and a static Prisma import (IA's/Spoken
 * English's equivalent CLIs both shipped with a broken dynamic import that
 * crashed on every run — built correctly from the start here).
 *
 *   npm run mock:assign-keys -- --dir "path/to/csvs"
 *   npm run mock:assign-keys -- --dir "path/to/csvs" --dry-run
 *   npm run mock:assign-keys -- --file "path/to/one.csv"
 *
...
import { findCsvFiles, loadMockCsv, writeMockCsv } from '../shared/csvLoader';
import { determineBucket } from '../layer1-verifier/checks';
import { bucketKey, type BucketPair } from '../shared/types';
import {
  TAGGED_HEADER,
  assignKeys,
  buildBucketIndex,
...
import { fetchBucketRows, indexFromDbRows, mergeBucketIndexes, type MockQuestionReader } from './dbIndex';
...
function resolveInputFiles(opts: CliOptions, positional: string[]): string[] {
  ...
  if (sources.length !== 1) {
    throw new UsageError(sources.length === 0 ? 'Choose what to tag: --dir <path>, or --file <path>.' : `Pick one way to choose files — got ${sources.join(' and ')}.`);
  }
...
async function main(): Promise<number> {
  ...
  program
    .name('mock:assign-keys')
    .description('Assigns a permanent source_key to every Mock question and writes tagged copies.')
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log('  Key-assignment tool — Mock source_key');
  console.log(`  Files: ${files.length}   Output: ${outBase}`);
  console.log(useDb ? '  Reading existing keys from the database (read-only).' : '  --no-db: existing database keys are NOT being consulted.');
  ...
  for (const filePath of files) {
    ...
    if (bucket === null) {
      outcomes.push({ fileName, bucket: null, outPath: null, kept: 0, reused: 0, assigned: 0, dropped: [], skipped: 0, error: 'not tagged — could not determine the (skill, sub_skill) bucket.' });
      continue;
    }
    ...
    if (index === undefined) {
      ...
      if (!useDb) {
        ...
        const dbRows = await fetchBucketRows(prisma as unknown as MockQuestionReader, bucket);
        const { index: fromDb, unkeyedRows, foreignKeys } = indexFromDbRows(dbRows, bucket);
        index = mergeBucketIndexes(fromDb, fromDisk);

        console.log(`  [db]   ${key} — ${dbRows.length} row(s) live, ${fromDb.keyByPrompt.size} matchable by text`);
        if (unkeyedRows > 0) {
    ...
    const updated: BucketIndex = { keyByPrompt: new Map(index.keyByPrompt), fileByKey: new Map(index.fileByKey), highestByPrefix: new Map(index.highestByPrefix) };
    ...
    const outPath = taggedOutputPath(outBase, filePath);

    if (!dryRun) writeMockCsv(outPath, TAGGED_HEADER, toTaggedRows(result.assignments));
  ...
  console.log(needsReview ? '  NEEDS REVIEW — see the notes above before verifying or importing.' : '  Clean. Next: npm run mock:verify -- --require-source-key --dir <output>');
