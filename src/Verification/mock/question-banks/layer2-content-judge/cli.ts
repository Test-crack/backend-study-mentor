/**
 * Layer 2 — Mock Question Content Judge.
 *
 ...
 * prompts are clear and genuinely discriminating (task_type-aware — see
 * prompts.ts), and transcribes Listening audio for manual reference. Calls
 * Gemini; costs money; caches by content hash. Never modifies a CSV, never
 * touches the database.
 *
 *   npm run mock:judge -- --dir <dir> --limit 20
 *   npm run mock:judge -- --dir <dir>
 *   npm run mock:judge -- --file "./a.csv" --no-cache
 *
...
function resolveInputFiles(opts: CliOptions, positional: string[]): string[] {
  ...
  if (sources.length !== 1) {
    throw new UsageError(sources.length === 0 ? 'Choose what to judge: --dir <path>, or --file <path>.' : `Pick one way to choose files — got ${sources.join(' and ')}.`);
  }
...
async function main(): Promise<number> {
  ...
  program
    .name('mock:judge')
    .description('Layer 2 content judge for Mock question CSVs. Read-only.')
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log('  Layer 2 — Mock Question Content Judge');
