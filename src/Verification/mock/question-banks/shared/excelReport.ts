/**
 * Writes the colored .xlsx Layer 1 report for Mock batches. Same shape and
 * palette as drills'/IA's — one Summary sheet plus one sheet per input CSV.
 */
...
export function sheetNameFor(file: FileResult, index: number, taken: Set<string>): string {
  ...
  const label = file.bucket ? `${file.bucket.skill}-${file.bucket.sub_skill}` : path.basename(file.fileName, path.extname(file.fileName));
...
function writeSummarySheet(workbook: ExcelJS.Workbook, run: RunResult, sheetNames: string[]): void {
  ...
  const title = sheet.addRow(['Layer 1 — Mock Question CSV Verifier']);
...
export async function writeRunReport(run: RunResult, outPath: string): Promise<string> {
  ...
  workbook.creator = 'Layer 1 Mock Question Verifier';
