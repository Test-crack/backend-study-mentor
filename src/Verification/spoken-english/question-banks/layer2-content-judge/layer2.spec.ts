...
// ---------------------------------------------------------------------------
// readQuestion on a well-formed row
// ---------------------------------------------------------------------------

// NOTE: the IELTS layer2.spec.ts this was forked from checked this against a
// real Layer1-verifier/__fixtures__ CSV — this fork has no fixtures directory
// (see layer1.spec.ts's header note), so this checks makeRow()'s row shape
// directly instead of reading one off disk. The behaviour under test
// (readQuestion parsing a well-formed row) is unaffected either way.
test('readQuestion accepts a well-formed row', () => {
  const row = makeRow();
