 ...
 *
 *   npm run drills:verify                                  (every level in drills/)
 ...
 *   npm run drills:verify -- --dir "C:\path\to\csvs" --match SPEAKING
 *   npm run drills:verify -- --file "./a.csv" --file "./b.csv"
 *
 * Exit codes:
 ...
 * (`advanced=50,else=200`). The per-level form exists so that one run can cover
...
function parseExpected(raw: string): ExpectedSpec {
  ...
  for (const part of raw.split(',')) {
    ...
    if (rawValue === undefined) {
      throw new UsageError(
        `--expected part "${part.trim()}" must look like level=count, e.g. advanced=50.`,
      );
    }
    ...
    if (level === null) {
      throw new UsageError(
        `--expected key "${key}" is not a level. Use beginner, intermediate, advanced, or else.`,
      );
    }
...
async function main(): Promise<number> {
  ...
  program
    ...
    .option('--level <name>', 'Check one level folder: beginner | intermediate | advanced')
