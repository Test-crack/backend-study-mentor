...
export const calculateModuleProgress = async (
  ...
  const totalResult = await prisma.$queryRaw<Array<{ total: bigint }>>`
    ...
    FROM "course_content_items_legacy" cci
    JOIN "module_concepts_legacy" mc ON cci."concept_id" = mc."concept_id"
  ...
  const completedResult = await prisma.$queryRaw<Array<{ completed: bigint }>>`
    ...
    FROM "user_content_progress_legacy" ucp
    JOIN "course_content_items_legacy" cci ON ucp."content_item_id" = cci."id"
...
export const calculateCourseProgress = async (
  ...
  >`
    ...
    FROM "course_modules_legacy" cm
    LEFT JOIN "user_module_progress_legacy" ump ON cm."module_id" = ump."module_id" 
...
export const getResumeData = async (userId: string, courseId: string) => {
  ...
  >`
    WITH user_content AS (
      ...
      FROM "user_content_progress_legacy" ucp
      JOIN "course_content_items_legacy" cci ON ucp.content_item_id = cci.id
      JOIN "module_concepts_legacy" mc ON cci.concept_id = mc.concept_id AND mc.module_id = ${courseModule.module_id}::uuid
      WHERE ucp.user_id = ${userId}::uuid
